...
from .domain_lexicon import correct_stt_text
from .tempo import stretch_stream
...
class Direction:
        ...
        try:
            async for event in self._session:
                ...
                if event.type == "partial":
                    ...
                    # Доменные ослышки правятся до чанкера и одинаково для
                    # каждой гипотезы — иначе LocalAgreement видит «новое» слово.
                    text = correct_stt_text(event.text, self.speaker.lang)
                    units = self._chunker.on_partial(text, now)
                    self._enqueue(units, now)
                    await self._emit_partial(text, now)
                    continue

                if event.type == "final":
                    if self._t_first_partial is None:
                        self._t_first_partial = now
                    units = self._chunker.on_final(correct_stt_text(event.text, self.speaker.lang), now)
                    self._enqueue(units, now)
    ...
    async def _process_unit(self, item: _Unit) -> None:
        ...
        try:
            ...
            # Перевод, ускоренный до ~1.25x, влезает в паузы живой речи
            # (эксп. 14); высота голоса не меняется. tempo=1.0 — сквозной путь.
            stream = stretch_stream(stream, self._tts_tempo)
