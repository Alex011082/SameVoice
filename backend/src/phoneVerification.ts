...
export type VerifyResult =
  ...
  | { ok: false; reason: "invalid_code" | "invalid_challenge" };
...
function sweepExpired(nowMs: number): void {
    ...
    for (const [token, verified] of verifiedPhones) {
        if (verified.expiresAtMs <= nowMs)
            verifiedPhones.delete(token);
    }
    for (const [phone, budget] of attemptBudgets) {
        if (nowMs - budget.windowStartMs >= ATTEMPT_WINDOW_MS)
            attemptBudgets.delete(phone);
    }
}
...
function spendAttempt(phone: string, nowMs: number): boolean {
    ...
    if (budget.used >= MAX_ATTEMPTS_PER_PHONE)
        return false;
    budget.used += 1;
    return true;
}
...
export function consumeVerifiedPhone(token: string, nowMs: number = Date.now()): string | null {
    const verified = verifiedPhones.get(token);
    verifiedPhones.delete(token);
    if (!verified || verified.expiresAtMs <= nowMs)
        return null;
