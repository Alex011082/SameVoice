    ...
    return async (app) => {
        ...
        app.post("/api/auth/phone/start", async (req, reply) => {
            ...
            if (!cfg.authDevCodeInResponse) {
                ...
                req.log.warn({ challengeId: challenge.challengeId, code: challenge.code }, "confirmation code issued (SMS is not wired; AUTH_DEV_CODE_IN_RESPONSE is off)");
            }
        ...
        app.post("/api/auth/seeded-login", async (req, reply) => {
            ...
            // Список посеянных выше уже проверен, но getUser типизирован как
            // «может не быть»: молчаливое падение здесь было бы 500 вместо 403.
            if (!user) {
                return reply
                    .code(403)
                    .send(apiError("forbidden", "only the seeded test identities can be signed into this way"));
