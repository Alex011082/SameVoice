...
interface SessionPayload {
  /** User id. The ONLY identifying field: no phone, no display name. */
  u: string;
  /** Issued-at, epoch seconds. */
  i: number;
  /** Expiry, epoch seconds. A token without one would be a permanent password. */
  e: number;
}
 ...
 */
export function readSession(secret: string, token: string, nowMs = Date.now()): Session | null {
  ...
  try {
    payload = JSON.parse(Buffer.from(parts[1]!, "base64url").toString("utf8")) as SessionPayload;
  } catch {
    return null;
  }
...
export interface CookieOptions {
  /** Off only for a plain-http local origin — see `cookieShouldBeSecure`. */
  secure: boolean;
