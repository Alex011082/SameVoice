...
# Предсказатель — основа скорости всего продукта: пока он не ответил, перевод
# обязан ждать конца фразы, а это самая длинная ступень задержки. Поэтому он
# НЕ стоит в очереди по одной задаче, как прочие сервисы.
#
# Замер 10.09.2026 показал, чем кончается лимит в единицу: тень спрашивает
# около 5 раз в секунду, прогон занимает ~300 мс, то есть успевает 3 — очередь
# росла всю минуту разговора, догадки опаздывали на 14 с, и ВСЕ 284 попытки
# умерли по таймауту. Ноль полезной работы.
#
# 8 одновременных прогонов: модель 0.6B занимает ~1,5 ГБ из 24, и восемь её
# копий по памяти проходят с большим запасом. Верхняя граница нужна не ради
# карты, а ради слуха: он живёт на той же карте, и предсказатель не должен
# съесть её целиком — тогда сломается сам разговор, ради которого он работает.
GPU_CONCURRENCY = concurrency_from_env("PREDICTOR_GPU_CONCURRENCY", default=8)
...
class PredictorEngine:
    ...
    def _load(self) -> float:
        ...
        with self._lock:
            ...
            # СВОЯ КАРТА. Предсказатель — основа скорости: пока он молчит,
            # перевод ждёт конца фразы. Делить карту со слухом он не должен —
            # 10.09.2026 при общей карте и очереди из одной задачи все 284
            # попытки за звонок умерли по таймауту, полезной работы ноль.
            #
            # По умолчанию берём ВТОРУЮ карту, когда она есть, и первую, когда
            # карта одна: под с одной картой обязан продолжать работать, просто
            # медленнее. PREDICTOR_DEVICE перекрывает выбор вручную.
            device = os.getenv("PREDICTOR_DEVICE", "").strip()
            if not device:
