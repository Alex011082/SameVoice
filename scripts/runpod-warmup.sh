...
post() {
  # НЕ `body="${3:-{}}"`. Bash закрывает подстановку на первой `}`, поэтому
  # значение по умолчанию получается `{`, а вторая `}` дописывается СНАРУЖИ —
  # и при переданном теле выходит `{"x":1}}`, то есть битый JSON и падение
  # прогрева. Пустой случай при этом работал, поэтому ошибка и дожила досюда.
