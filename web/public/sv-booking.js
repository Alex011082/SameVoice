...
(function () {
  ...
  /* Автосозвон: в назначенный срок инициатор брони набирает сам — «как
     телефонистка». Набирает именно ИНИЦИАТОР, чтобы обе стороны не позвонили
     друг другу одновременно; вторая сторона просто снимает трубку (бот — сам).
     Защёлка в localStorage: одна бронь — один автонабор. */
  function maybeAutoCall(mine) {
    if (!me) return;
    var now = Date.now();
    for (var i = 0; i < mine.length; i++) {
      var b = mine[i];
      if (b.state !== 'confirmed' || b.byUserId !== me) continue;
      if (now < b.startsAt - 3000 || now > b.startsAt + 120000) continue;
      var key = 'sv-autocall-' + b.id;
      try { if (localStorage.getItem(key)) continue; } catch (e) {}
      var card = document.querySelector('#contact-list li.card[data-user-id="' + b.withUserId + '"]');
      var btn = card && card.querySelector('.card-acts .btn');
      if (!btn) continue;
      try { localStorage.setItem(key, '1'); } catch (e) {}
      // Флаг «время брони пришло»: перехват «позвонить боту» обязан звонить
      // сразу, а не заводить новую бронь — иначе автосозвон ходит по кругу.
      window.__svAutoCallNow = b.id;
      btn.click();
      setTimeout(function () { window.__svAutoCallNow = null; }, 4000);
      return;
    }
  }
  setInterval(function () { maybeAutoCall(lastMine); }, 5000);
  var lastMine = [];
