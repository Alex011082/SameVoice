...
export function callErrorText(err: unknown): string {
  ...
  switch (failure.code) {
    ...
    case 'unauthorized':
      return 'Вход устарел — звонок не начался. Нажмите «сменить» вверху и войдите по номеру заново.';
    case 'forbidden':
      // Ссылка на звонок, открытая не тем профилем. Раньше такого ответа не
      // было вовсе: `?me=` в ссылке делал открывшего тем, кем ссылка скажет.
      return 'Этот звонок не ваш: ссылка выписана на другой профиль.';
    default:
...
export function contactsErrorText(err: unknown): string {
  ...
  // Обычный, а не редкий случай: профили живут в памяти сервера, так что
  // перезапуск backend'а делает сессию в браузере недействительной. Человеку
  // надо сказать «войдите заново», а не пересказывать английский ответ сервера.
  if (failure.code === 'unauthorized') {
    return 'Вход устарел. Нажмите «сменить» вверху и войдите по номеру заново.';
  }
  if (failure.code === 'forbidden') {
    return 'Эта страница открыта под другим профилем. Нажмите «сменить» вверху и войдите заново.';
  }
 ...
 * «Добавить по номеру».
 *
 * Отдельная функция, а не ветка в contactsErrorText: половина этих ответов
 * существует только у этой ручки, и одна из них — про приватность чужого
 * номера, а не про поломку.
 *
 * Здесь НЕТ и не может быть текста «такого номера нет». Сервер отвечает
 * одинаково на зарегистрированный и на незнакомый номер намеренно: ручка,
 * которая различает эти два случая, превращается в справочник «кто здесь
 * есть» — та самая утечка, которой до 31.08.2026 был GET /api/users, только
 * запрос идёт по номеру телефона. Тот список с тех пор сузили до четырёх
 * посевных личностей, и сузили на сервере. Если бы клиент дорисовывал такое
 * различие сам, он бы вернул утечку обратно — молча, потому что в backend'е всё
 * было бы правильно.
 */
export function addByPhoneErrorText(err: unknown): string {
  const failure = toFailure(err);
  if (isNetwork(failure)) {
    return 'Сервер не отвечает — контакт не добавлен. Попробуйте ещё раз.';
  }
  switch (failure.code) {
    case 'rate_limited':
      // Не «вы что-то сделали не так»: ограничение стоит ради чужих номеров, а
      // не ради нагрузки, и человеку честнее сказать, почему его придержали.
      return 'Слишком много добавлений подряд. Это ограничение защищает чужие номера от перебора — попробуйте позже.';
    case 'unauthorized':
      return 'Вход устарел. Нажмите «сменить» вверху и войдите по номеру заново.';
    case 'bad_request':
      return BAD_REQUEST.phone;
    case 'route_missing':
      // Тот же приём, что у звонков и «судьи»: старая сборка backend'а — это
      // не ошибка человека, и предлагать ему «попробовать ещё раз» бессмысленно.
      return 'Эта сборка сервера ещё не умеет добавлять по номеру. Обновите backend.';
    default:
      return `Контакт не добавлен. Сервер ответил: ${failure.message}`;
  }
}
